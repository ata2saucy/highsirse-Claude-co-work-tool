...
- `{Building} Rental Comps _vACTIVE.xlsx` — v2 format specified in CLAUDE.md, **six sheets**: Output (with **two product-type views — condos-only and apartments-only**, and the recommended-rent summary folded in from the former Subject & Conclusion sheet) · Building Summary · Data_Summary · **`RD Condos` and `RD Apartments`** (RD = Raw Data; two identical-structure sheets, 45 columns A:AS — incl. Date Scraped + four SF-validation columns) · Floor Plans (18-column session plan log); live formulas (incl. the LINEST parking regression), zero recalc errors (run `tools/qa_workbook.py` before delivery). Condos and apartments are never blended; apartment rent/listing data may come from Apartments.com + Canadian rental sites (apartment rows only — those sources never supply an SF figure, and apartment interior SF clears the SAME strict exact-or-blank bar). Asking (listed) vs achieved (leased) rent is kept explicit, with the "selling as vs sold for" spread shown in each Output view.
